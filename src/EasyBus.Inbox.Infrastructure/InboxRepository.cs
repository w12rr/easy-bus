...
public class InboxRepository : IInboxRepository, IAsyncDisposable
{
    ...
    private readonly InboxSqlQueriesOptions _queriesOptions;

    public InboxRepository(IOptions<InboxConnectionStringOptions> connectionStringOptions,
        IOptions<InboxSqlQueriesOptions> queriesOptions)
    {
        _connectionString = connectionStringOptions.Value.ConnectionString;
        _queriesOptions = queriesOptions.Value;
    }
    ...
    public async Task CreateTable(CancellationToken cancellationToken)
    {
        ...
        await using var cmd = new SqlCommand(_queriesOptions.CreateTable, transaction.Connection, transaction);
    ...
    {
        var transaction = await StartConnectionIfNotStarted(cancellationToken);
        await using var cmd = new SqlCommand(_queriesOptions.DeleteOldProcessed, transaction.Connection, transaction);
    ...
    {
        var transaction = await StartConnectionIfNotStarted(cancellationToken);
        await using var cmd = new SqlCommand(_queriesOptions.GetUnprocessed, transaction.Connection, transaction);
    ...
    {
        var transaction = await StartConnectionIfNotStarted(cancellationToken);
        await using var cmd = new SqlCommand(_queriesOptions.SetReceived, transaction.Connection, transaction);
    ...
    {
        var transaction = await StartConnectionIfNotStarted(cancellationToken);
        await using var cmd = new SqlCommand(_queriesOptions.UpdateTries, transaction.Connection, transaction);
        cmd.Parameters.AddWithValue("@Id", id);
    ...
    {
        var transaction = await StartConnectionIfNotStarted(cancellationToken);
        await using var cmd = new SqlCommand(_queriesOptions.Insert, transaction.Connection, transaction);
